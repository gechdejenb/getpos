...
class RestockController extends BaseController
{
    ...
    private function formatItemsLines(array $items)
    {
        $lines = [];
    ...
    private function getTelegramAdminChatId()
    {
        $envChatId = config('services.telegram.admin_chat_id');
        if ($envChatId) {
            return $envChatId;
        }

        $settings = \App\Models\Setting::where('deleted_at', '=', null)->first();
        return $settings ? $settings->telegram_admin_chat_id : null;
    }
    ...
    public function notifyRequesterStatus(RestockRequest $restockRequest, $status)
    {
        ...
        $lines = $this->formatItemsLines(is_array($restockRequest->items) ? $restockRequest->items : []);
    ...
    public function callbackExternal(RestockRequest $restockRequest, $event)
    {
        if (!$restockRequest->callback_url) {
            return;
        }

        try {
            Http::timeout(5)->post($restockRequest->callback_url, [
                'event' => $event,
                'request_id' => $restockRequest->id,
                'status' => $restockRequest->status,
                'transfer_id' => $restockRequest->transfer_id,
                'external_request_id' => $restockRequest->external_request_id,
            ]);
        } catch (\Throwable $e) {
    ...
    public function store(Request $request)
    {
        ...
        $restockRequest = RestockRequest::create([
            ...
            'requested_by_system' => $request->input('requested_by_system'),
            'requested_by_name' => $request->input('requested_by_name'),
        ...
        $lines = $this->formatItemsLines($request->items);
        ...
        $adminChatId = $this->getTelegramAdminChatId();
        if ($adminChatId) {
            $telegram = new TelegramService();
            $telegram->sendMessage([
                'chat_id' => $adminChatId,
        ...
        $this->callbackExternal($restockRequest, 'created');
    ...
    public function approve(Request $request, $id)
    {
        ...
        $this->callbackExternal($restockRequest, 'approved');
    ...
    public function reject(Request $request, $id)
    {
        ...
        $this->callbackExternal($restockRequest, 'rejected');
    ...
    public function complete(Request $request, $id)
    {
        ...
        $this->callbackExternal($restockRequest, 'completed');
